\begin{tikzpicture}[shorten >=1pt, ->, draw=black!50, node distance=1.5cm and 3.5cm, align=center]
    ...
    \node[dense] (norm_pre_dense) [right of=node_features, xshift=3cm] {Normalization \\ \& Pre-dense Layers};
    ...
    % Outputs
    \node[output] (node_output) [right of=post_dense, xshift=3cm] {Node Predictions};
    \node[output] (edge_output) [below of=node_output] {Edge Predictions};

    % Losses
    \node[loss] (node_loss) [right of=node_output, xshift=3cm] {Node Loss};
    \node[loss] (edge_loss) [below of=node_loss] {Edge Loss};

...
\begin{itemize}
    \item \textbf{Node Features:} A matrix $\in \mathbb{R}^{N_v \times 3}$. Each node feature includes the lower and upper limits for injected flow, as well as demanded flow.
    \item \textbf{Node Laplacian:} An adjacency matrix of size $N_e \times N_e$ encoding the graph structure of the nodes.
    \item \textbf{Edge Laplacian:} A matrix of size $N_e \times N_e$ encoding the connections between edges.
    \item \textbf{Incidence Matrix:} A matrix of size $N_v \times N_e$ representing the node-edge incidence relationship, mapping the flow of gas between nodes through edges.
    \item \textbf{Edge Features:} A matrix $\in \mathbb{R}^{N_e \times 5}$ that includes the features of pipelines and compressors in the gas network. Each edge feature includes the $K$ constant, the maximum compression ratio $\beta$, and the upper and lower flow limits.
\end{itemize}

...
% \begin{equation}
%     \mathbf{H}^{(l+1)}_e = \sigma\left(\left[\mathbf{T}^\top \Phi\left(\mathbf{H}^{(l)}_v \mathbf{p}_v\right) \mathbf{T}\right] \odot \tilde{\mathbf{A}}_e \, \mathbf{H}^{(l)}_e \mathbf{W}_e\right),
% \end{equation}
...
% In this experimental setup, different network operation scenarios were generated by perturbing the users' consumption values with added noise and subsequently solving each scenario using the linear constrained optimization model presented in the previous section. The noise levels range from 5\% to 25\%, applied to the parameters of the networks to simulate diverse operating conditions. 

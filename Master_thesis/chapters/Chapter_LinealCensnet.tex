...
\begin{table}
    ...
    \begin{tabular}{|l|c|c|c|c|}
        ...
        CensNet (N) & \( 0.000 \pm 18.144 \) & \( 23.760 \pm 14.042 \) & \( 5.710 \pm 16.850 \) & \( 0.650 \pm 0.442 \) \\ \hline
        CensNet (N+E) & \( 0.070 \pm 18.081 \) & \( 0.080 \pm 19.884 \) & \( 0.080 \pm 1.171 \) & \( 0.640 \pm 0.444 \) \\ \hline
    \end{tabular}
...
\begin{table}
    ...
    \begin{tabular}{|l|c|c|c|c|}
        \hline
        Method & Node Error & Edge Error & Balance Error & Time \\ \hline
        CensNet (N) & \( 0.030 \pm 69.551 \) & \( 62.610 \pm 81.630 \) & \( 0.040 \pm 60.837 \) & \( 4.880 \pm 5.590 \) \\ \hline
        CensNet (N+E) & \( 0.010 \pm 69.438 \) & \( 0.070 \pm 115.259 \) & \( 0.010 \pm 23.398 \) & \( 4.870 \pm 5.591 \) \\ \hline
    \end{tabular}
...
When compared to the APOPT optimization model, the CensNet achieves comparable performance in terms of flow predictions, particularly when both nodal and edge losses are used. In the 8-node case, for instance, the CensNet (N+E) model achieves a significant reduction in edge error compared to the nodal-only model, with a mean edge difference of \(0.08\) units versus \(23.76\), as shown in \Cref{tab:lineal_dummy_results_diffs}. Additionally, the gas balance error improves substantially, from \(5.71\) to \(0.08\), indicating improved mass conservation when edge information is incorporated into training.


For the 63-node case, the advantage of using both nodal and edge losses remains consistent. While the nodal-only model (CensNet (N)) yields a large edge error difference of \(62.61\), this is reduced to just \(0.07\) by the CensNet (N+E) model, as shown in \Cref{tab:lineal_col_results_diffs}. Moreover, the balance error also decreases from \(0.04\) to \(0.01\), reinforcing the benefit of training with comprehensive physical constraints. 
